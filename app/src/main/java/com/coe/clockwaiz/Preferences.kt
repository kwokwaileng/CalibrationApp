...
abstract class Preferences {
    ...
    /*fun positionOffSetPref(prefKey: String? = null, defaultValue: Set<PositionOffset> = HashSet<PositionOffset>)) = PositionOffSetPrefDelegate(prefKey, defaultValue)

    inner class PositionOffSetPrefDelegate(prefKey: String? = null, val defaultValue: Set<PositionOffset>) : PrefDelegate<Set<String>>(prefKey) {
        override fun getValue(thisRef: Any?, property: KProperty<*>): Set<PositionOffset>{
            val json:Set<String> = HashSet()
            val offset:Set<PositionOffset> = HashSet()
            val positionOffSetStr = prefs.getStringSet(prefKey ?: property.name, json)
            Gson().fromJson(positionOffSetStr.first().toString(), PositionOffset::class)

            return offset
        }

         fun setValue(thisRef: Any?, property: KProperty<*>, value: Set<PositionOffset>) {

            val json:Set<String> = HashSet()
             json.plus(Gson().toJson(value))
            prefs.edit().putStringSet(prefKey ?: property.name, json).apply()
            onPrefChanged(property)
        }
    }
*/
